...
def _resolve_schema(schema: str, schema_name: str = None) -> str:
...
async def list_tables(schema: str = "public", schema_name: str = None) -> str:
    ...
    try:
        async with db.db_pool.connection() as conn:
            rows = await conn.fetch(
                table_queries["tables_in_schema"], target_schema
            )
        tables = [{
            "name": r["table_name"],
        ...
        async with db.db_pool.connection() as conn:
            ...
            if our_oid:
                pk_rows = await conn.fetch(
                    table_queries["primary_keys"], our_oid
                )
...
async def get_table_count(
    ...
    try:
        async with db.db_pool.connection() as conn:
            rows = await conn.fetch(
                table_queries["table_count"], target_schema, table_name
            )
...
async def get_table_indexes(
    ...
    try:
        async with db.db_pool.connection() as conn:
            # Get table OID
            oid_result = await conn.fetchval(
                table_queries["table_oid"], target_schema, table_name
            )
            ...
            # Get indexes
            rows = await conn.fetch(
                table_queries["indexes"], oid_result
            )
        indexes = [{
            "name": r["index_name"],
            "unique": r["is_unique"],
